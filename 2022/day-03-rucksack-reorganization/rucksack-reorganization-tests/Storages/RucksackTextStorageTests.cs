...
{
    public class RucksackTextStorageTests
    {
        [TestCaseSource(typeof(RucksackTests.RucksackDataSource))]
        public void WhenReadLines_ThenShouldReturnRucksacks_WithCorrectRepeatedSymbols(string content, char expected)
        {
            ...
            textMock.Setup(mock => mock.Lines()).Returns(new[] { content });
            var storage = new RucksackTextStorage(textMock.Object);
